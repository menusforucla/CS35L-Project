...
function scrapeHours(): Promise<void> {
  try {
    axios.get(diningURLs[0]).then(({ data }) => {
...
async function scrapeMenu(menuOption: number): Promise<void> {
  try {
    ...
    console.log(
      `Error fetching data from ${diningURLs[menuOption]}: ${error.message}`,
    );
  }
}
...
async function scrapeNutrition(url: string): Promise<NutritionalInfo> {
  try {
    ...
    console.error(
      `Error fetching nutritional information from ${url}: ${error.message}`,
    );
    throw error;
  }
}
